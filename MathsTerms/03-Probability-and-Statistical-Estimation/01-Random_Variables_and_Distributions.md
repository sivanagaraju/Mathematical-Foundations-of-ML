...
> - **Required Now:** [Probability Basics & Axioms](../03-Probability-and-Statistical-Estimation/00-Probability_Basics_and_Axioms.md) (Sample space $\Omega$, events $\mathcal{F}$, probability measure $P$, Kolmogorov axioms) · [Functions, Derivatives & Rules](../02-Multivariate-Calculus-and-Optimization/01-Functions_Derivatives_and_Rules.md) (Integration as area under curves, continuous density $\int p(x)dx=1$, derivatives $p(x) = F'(x)$)  
