...
> - **Deep Rigor / Researcher:** Read all sections sequentially including Section 8 (Theoretical Formulations), Section 9 (Proofs of Kantorovich-Rubinstein Duality), and Section 12 (Diagnostic Checks).
...
> - Prove why Wasserstein distance produces constant, clean gradients where Jensen-Shannon divergence experiences gradient vanishing.
> - Implement the WGAN-GP Gradient Penalty loss in PyTorch autograd.
...
| **Gradient on Disjoint Manifolds** | **Constant non-zero:** $\pm 1.0$ (Clean guidance!) | **Zero:** $\nabla Loss = 0$ (Vanishing gradient!) | **Infinite / Undefined** (Gradient explosion!) | **Zero:** $\nabla Loss = 0$ (Discontinuous step) |
...
abla D\|_2 pprox 1$ | 1-Lipschitz penalty is evaluated only on straight lines between points, not across all $\mathbb{R}^D$. |
...
2. **Find the Optimal Dual Witness Function:** According to Kantorovich-Rubinstein duality:
   $$W_1(P, Q) = \sup_{f: \|f\|_L \le 1} \left( \mathbb{E}_{x \sim P}[f(x)] - \mathbb{E}_{y \sim Q}[f(y)] ight) = \sup_{f: \|f\|_L \le 1} [f(1) - f(5)]$$
   Construct an explicit 1-Lipschitz function $f^*(x)$ that achieves this supremum and evaluate $f^*(1) - f^*(5)$.
3. **Contrast with JSD under Spatial Translation:** Suppose $Q$ is shifted from $x = 5$ to $x = 100$. Compare how $W_1(P, Q)$ and $	ext{JSD}(P \parallel Q)$ respond to this shift, explaining why Wasserstein distance prevents gradient vanishing.
...
abla_x W_1 = -1$ directing the generator towards the data.
   - **Jensen-Shannon Divergence:** $	ext{JSD}(P \parallel Q) = \ln 2$ at $x = 5$, and $	ext{JSD}(P \parallel Q') = \ln 2$ at $x = 100$. JSD saturates at the exact same constant value, yielding zero gradient $
abla_x 	ext{JSD} = 0$.
...
| :--- | :--- | :--- | :--- | :--- |
| [Gabriel Peyré & Marco Cuturi: Computational Optimal Transport (2019)](https://optimaltransport.github.io/) | Comprehensive Open Textbook | Complete mathematical foundation of Monge-Kantorovich problems, Sinkhorn divergences, and Wasserstein geometry. | Definitive textbook for researchers in optimal transport and generative models. | ✅ Active Open Access Classic |
| [Martin Arjovsky, Soumith Chintala, Léon Bottou: Wasserstein GAN (2017)](https://arxiv.org/abs/1701.07875) | Seminal Foundation Paper | Introduces the Kantorovich-Rubinstein dual formulation to solve GAN training instability. | Essential reading for understanding modern generative modeling theory. | ✅ Published ICML Classic |
| [Ishaan Gulrajani et al.: Improved Training of Wasserstein GANs (2017)](https://arxiv.org/abs/1704.00028) | Applied Deep Learning Paper | Introduces the Gradient Penalty (WGAN-GP) to enforce the 1-Lipschitz condition reliably without weight clipping. | Canonical paper for practical production Wasserstein training. | ✅ Published NeurIPS Classic |
| [Marco Cuturi: Sinkhorn Distances: Lightspeed Computation of Optimal Transport (2013)](https://arxiv.org/abs/1306.0895) | Seminal Algorithmic Paper | Introduces entropic regularization enabling GPU-accelerated matrix-scaling optimal transport. | Essential reading for large-scale optimal transport applications. | ✅ Published NeurIPS Classic |
| [Cédric Villani: Optimal Transport: Old and New (Springer 2009)](https://link.springer.com/book/10.1007/978-3-540-71050-9) | Advanced Mathematical Treatise | The Fields Medalist's comprehensive mathematical treatise on optimal transport and Riemannian geometry. | Consult for deep, rigorous measure-theoretic and geometric proofs. | ✅ Published Academic Classic |
| [POT: Python Optimal Transport Library](https://pythonot.github.io/) | Official Engineering Library | Fast GPU-accelerated library for Sinkhorn distances, Wasserstein barycenters, and empirical optimal transport. | Use for benchmarking and implementing optimal transport algorithms in Python. | ✅ Active Open Source Library |
