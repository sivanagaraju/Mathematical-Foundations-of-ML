...
> - **[Latent Variable Models](./05-Latent_Variable_Models.md)** — Intractable marginal evidence $\ln p(x) = \ln \int p(x, z) dz$ and unobserved latent factors
> - **[Convexity & Jensen's Inequality](../01-Primal-Analysis-and-Foundations/03-Convexity_and_Jensens_Inequality.md)** — Jensen's bound on concave natural logarithm $\ln \mathbb{E}[X] \ge \mathbb{E}[\ln X]$
> - **[KL Divergence](../05-Information-Theory-and-Divergences/02-KL_Divergence.md)** — Variational posterior regularizer $D_{\text{KL}}(q_\phi(z \mid x) \parallel p(z))$ matching the Gaussian prior
>
