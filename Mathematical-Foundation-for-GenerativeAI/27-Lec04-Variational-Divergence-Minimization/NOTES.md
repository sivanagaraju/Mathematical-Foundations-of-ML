...
| :--- | :--- | :--- | :--- |
| $\mathcal{D} = \{x_i\}_{i=1}^n$ | Real Data Sample Cloud | `real_batch = next(data_loader)` | Empirical dataset samples drawn from unknown $p_{\text{data}}(x)$. |
| $z_j \sim \mathcal{N}(0, I)$ | Latent Gaussian Noise | `z = torch.randn(batch_size, latent_dim)` | Random seed vector providing all stochasticity. |
| $G_\theta(z)$ | Deterministic Generator Network | `fake_batch = generator(z)` | Maps noise $z$ to synthetic data $\hat{x} \sim p_\theta$. |
| $p_{\text{data}}(x), p_\theta(x)$ | Unknown Probability Densities | *Intractable (Never evaluated directly)* | The theoretical distributions being aligned. |
| $f(u)$ | Convex Generator Function | `f = lambda u: u * torch.log(u)` | Determines which $f$-divergence is being minimized. |
| $f^*(t)$ | Fenchel Convex Conjugate | `f_star = lambda t: torch.exp(t - 1.0)` | Dual penalty applied to critic output on fake samples. |
| $\operatorname{dom}(f^*)$ | Domain of Conjugate Function | `activation_head` (e.g. Identity, `-exp`, Tanh) | Sets the valid output range of the Critic network. |
| $T_w(x)$ | Critic / Test Probe Network | `t_real = critic(real_batch)` | Parameterizes the function space $\mathcal{T}$ via weights $w$. |
| $\mathbb{E}_{x \sim p_{\text{data}}}[T_w(x)]$ | Data Expectation Term | `torch.mean(critic(real_batch))` | First term in the variational lower bound. |
| $\mathbb{E}_{z \sim p_Z}[f^*(T_w(G_\theta(z)))]$ | Generator Expectation Term | `torch.mean(f_star(critic(generator(z))))` | Second term in variational bound (evaluated via LOTUS). |
| $\mathcal{J}(\theta, w)$ | Variational Minimax Objective | `loss_critic = -(term1 - term2)` | The two-player zero-sum score surface. |
