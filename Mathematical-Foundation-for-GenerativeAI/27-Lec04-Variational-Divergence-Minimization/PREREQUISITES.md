...
| :--- | :--- | :--- | :--- | :--- |
| **$\mathcal{D} = \{x_i\}_{i=1}^n$** | Empirical Dataset Sample Cloud | A directory of real training images on your hard drive | An album of authentic Polaroid photographs. | [Probability Basics & Axioms](../../MathsTerms/01-Primal-Analysis-and-Foundations/01-Probability_Basics_and_Axioms.md) |
...
| **$z \sim \mathcal{N}(0, I)$** | Latent Noise Prior | Random Gaussian seed vector generated via `torch.randn` | Uncut marble stone ready for sculpting. | [Common Probability Distributions](../../MathsTerms/04-Probability-and-Statistical-Estimation/02-Common_Probability_Distributions.md) |
| **$\mathbb{E}_{x \sim p}[h(x)]$** | True Population Expectation | $\int h(x) p(x) dx$ (Theoretical expected value over infinite data) | The true average height of all 8 billion humans on Earth. | [Random Variables & Distributions](../../MathsTerms/04-Probability-and-Statistical-Estimation/01-Random_Variables_and_Distributions.md) |
| **$\frac{1}{n}\sum_{i=1}^n h(x_i)$** | Monte Carlo Sample Average | Empirical batch mean computed via `torch.mean()` | Polling 1,000 people to estimate the true global average. | [Probability Basics & Axioms](../../MathsTerms/01-Primal-Analysis-and-Foundations/01-Probability_Basics_and_Axioms.md) |
| **$\text{LOTUS}$** | Law of Unconscious Statistician | Evaluating $\mathbb{E}[h(G(Z))]$ using noise samples $z$ without finding $p_G$ | Tasting pasta without calculating the physics of the extruder. | [Random Variables & Distributions](../../MathsTerms/04-Probability-and-Statistical-Estimation/01-Random_Variables_and_Distributions.md) |
| **$f^*(t)$** | Fenchel Convex Conjugate | Dual function $\sup_u \{ut - f(u)\}$ measuring intercept of tangent slope $t$ | A laser scanner recording the boundary envelope of a bowl. | [Fenchel Conjugate & Dual Reps](../../MathsTerms/01-Primal-Analysis-and-Foundations/05-Fenchel_Conjugate_and_Dual_Representations.md) |
| **$\operatorname{dom}(f^*)$** | Domain of Conjugate Function | The valid interval of numbers that can be fed into $f^*(t)$ | The legal voltage range of an electronic sensor. | [Fenchel Conjugate & Dual Reps](../../MathsTerms/01-Primal-Analysis-and-Foundations/05-Fenchel_Conjugate_and_Dual_Representations.md) |
| **$T(x) \in \mathcal{T}$** | Variational Test Function / Probe | Neural network $T_w(x)$ outputting scalar values in $\operatorname{dom}(f^*)$ | A metal detector sweeping over the ground for gold. | [Fenchel Conjugate & Dual Reps](../../MathsTerms/01-Primal-Analysis-and-Foundations/05-Fenchel_Conjugate_and_Dual_Representations.md) |
| **$\sup$ vs $\max$** | Supremum vs Maximum | Least upper bound (may be approached asymptotically without being reached) | The speed of light $c$ (universal ceiling you cannot exceed). | [Convexity & Jensen's Inequality](../../MathsTerms/01-Primal-Analysis-and-Foundations/03-Convexity_and_Jensens_Inequality.md) |
| **$\min_\theta \max_w \mathcal{J}$** | Minimax Saddle Optimization | Two-player zero-sum adversarial game between Generator and Critic | A chess match between a master forger and an art detective. | [f-Divergence](../../MathsTerms/05-Information-Theory-and-Divergences/04-f_Divergence.md) |
