...
| :--- | :--- | :--- | :--- | :--- |
| **$\mathcal{J}(\theta, w)$** | Shared Scalar Minimax Objective | A single scalar output tensor computed in PyTorch | The elevation of a point on a mountain pass saddle. | [Jensen-Shannon Divergence](../../MathsTerms/Jensen_Shannon_Divergence.md) |
| **$\theta \in \mathbb{R}^{p}$** | Generator Parameter Vector | `generator.parameters()` (weights & biases of $G$) | The internal mechanical dials of a pasta extruder. | [Autoregressive Models](../../MathsTerms/Autoregressive_Models.md) |
| **$w \in \mathbb{R}^{q}$** | Critic / Discriminator Parameter Vector | `critic.parameters()` (weights & biases of $T$ or $D$) | The magnifying glass and ruler settings of a forensic detective. | [Jensen-Shannon Divergence](../../MathsTerms/Jensen_Shannon_Divergence.md) |
| **$\arg\min_\theta \max_w \mathcal{J}$** | Minimax Saddle-Point Optimization | Alternating PyTorch optimizer steps (`opt_D`, `opt_G`) | A competitive zero-sum board game between two grandmasters. | [Minimax Games & GANs](../../MathsTerms/Minimax_Game_and_GANs.md) |
| **$z \sim \mathcal{N}(0, I_k)$** | Latent Noise Prior ($k$-dimensional) | `z = torch.randn(batch_size, latent_dim)` | A lump of unshaped modeling clay. | [Common Probability Distributions](../../MathsTerms/Common_Probability_Distributions.md) |
| **$G_\theta(z) = \hat{x}$** | Push-Forward Deterministic Map | `fake_batch = generator(z)` | Extruding plain dough through a star-shaped nozzle into pasta. | [Autoregressive Models](../../MathsTerms/Autoregressive_Models.md) |
| **$T_w(x)$** | General VDM Statistical Critic | `critic_out = critic_net(x)` where output $\in \operatorname{dom}(f^*)$ | A food critic writing an unconstrained numerical score on a napkin. | [Fenchel Conjugate & Dual Reps](../../MathsTerms/Fenchel_Conjugate_and_Dual_Representations.md) |
| **$D_w(x)$** | Binary Classifier / Discriminator | `prob_real = torch.sigmoid(critic_net(x))` $\in (0, 1)$ | A border control officer stamping a passport: "Admit (1) or Deny (0)". | [Jensen-Shannon Divergence](../../MathsTerms/Jensen_Shannon_Divergence.md) |
| **$\sigma_f(v)$** | $f$-Divergence Lego Activation | Custom final activation module mapping $\mathbb{R} \to \operatorname{dom}(f^*)$ | A specialized mechanical adapter fitting a square peg into a round hole. | [Fenchel Conjugate & Dual Reps](../../MathsTerms/Fenchel_Conjugate_and_Dual_Representations.md) |
| **$\operatorname{dom}(f^*)$** | Domain of Fenchel Conjugate | Legal interval of numbers for $f^*(t)$ (e.g., $\mathbb{R}_-$ for GAN) | The allowed input voltage range of an electronic voltmeter. | [Fenchel Conjugate & Dual Reps](../../MathsTerms/Fenchel_Conjugate_and_Dual_Representations.md) |
| **$\sigma(v) = \frac{1}{1+e^{-v}}$** | Standard Logistic Sigmoid | `torch.sigmoid(v)` squashing $(-\infty, +\infty) \to (0, 1)$ | A pressure valve restricting water flow between 0% and 100%. | [Softmax](../../MathsTerms/Softmax.md) |
| **$\frac{1}{B}\sum_{i=1}^B \log D(x_i)$** | Monte Carlo Batch Sample Average | `torch.mean(torch.log(D_real))` | Asking 64 citizens their opinion to estimate a nationwide election. | [Probability Basics & Axioms](../../MathsTerms/Probability_Basics_and_Axioms.md) |
| **$y \in \mathbb{R}^c$** | Conditioning Semantic Vector | One-hot tensor `F.one_hot(label, 10)` or text embedding | A restaurant order slip specifying "Table 4 requests a Pepperoni Pizza". | [Joint, Marginal & Conditional Dist](../../MathsTerms/Joint_Marginal_Conditional_Dist.md) |
| **$[z; y]$** | Tensor Concatenation | `torch.cat([z, y], dim=1)` | Stapling an order slip to a blank sheet of paper before cooking. | [Joint, Marginal & Conditional Dist](../../MathsTerms/Joint_Marginal_Conditional_Dist.md) |
