...
| :--- | :--- | :--- | :--- | :--- |
| $\mathbf{X} \in \mathbb{R}^{N \times C \times H \times W}$ | 4D Mini-Batch Tensor | `x = torch.randn(N, C, H, W)` | Standard 4D image layout ($N$=batch, $C$=channels, $H$=height, $W$=width). | [Tensors & Shapes](../../MathsTerms/01-Linear-Algebra-Geometry-and-Tensors/04-Tensors_and_Shapes.md) |
...
| $\text{Flatten}$ | Multi-Axis Unrolling | `torch.flatten(x, 1)` | Flattens 3D feature cube into 1D vector ($C \cdot H \cdot W$). | [Tensors & Shapes](../../MathsTerms/01-Linear-Algebra-Geometry-and-Tensors/04-Tensors_and_Shapes.md) |
| $\mathbf{z} \in \mathbb{R}^{10}$ | Unnormalized Class Logits | `logits = model(x)` | 10-class scores fed directly to `nn.CrossEntropyLoss`. | [Softmax](../../MathsTerms/02-Multivariate-Calculus-and-Optimization/06-Softmax.md) |
| $\hat{c} = \arg\max_k z_k$ | Hard Predicted Class | `torch.argmax(logits, dim=1)` | Predicted class index used for accuracy evaluation. | [Argmax & Argmin](../../MathsTerms/02-Multivariate-Calculus-and-Optimization/07-Argmax.md) |
