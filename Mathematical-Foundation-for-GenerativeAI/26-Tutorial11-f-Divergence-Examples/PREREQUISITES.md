...
| :--- | :--- | :--- | :--- |
| **$P, Q$** | Probability Measures / Laws | Probability distribution objects that assign mass in $[0, 1]$ to regions | The total volume of water poured into distinct buckets. |
| **$p(x), q(x)$** | Probability Density Functions (PDFs) | Continuous height functions whose area gives probability | The height of a sand dune at a specific coordinate $x$. |
| **$R(x) = \frac{p(x)}{q(x)}$** | Likelihood / Density Ratio | Relative likelihood ratio comparing distribution $P$ against $Q$ at $x$ | A chemical litmus scale comparing acidity vs alkalinity. |
| **$P \ll Q$** | Absolute Continuity | $Q(A) = 0 \implies P(A) = 0$ (License to divide $p/q$) | Map B must show every street that Map A draws a building on. |
| **$f: \mathbb{R}_+ \to \mathbb{R}$** | Divergence Generator Function | Convex function satisfying $f(1)=0$ shaping the divergence spring | A bowl-shaped spring whose tension measures distribution gap. |
| **$f''(u) \ge 0$** | Convexity | Function curves upward; chords lie strictly above the curve | A smooth skate park ramp whose bottom dips below its rim. |
| **$\mathbb{E}_Q[g(X)]$** | Expectation under Measure $Q$ | $\int q(x) g(x) dx$ (Integral weighted by density $q$) | A weighted average restaurant bill split among diners. |
| **$D_f(P \parallel Q)$** | $f$-Divergence Functional | $\int q(x) f(p(x)/q(x)) dx = \mathbb{E}_Q[f(R)]$ | The total stretch energy stored in the divergence spring. |
| **$Q\text{-almost surely}$** | Measure-Theoretic Null Set Equality | True everywhere except on a subset of zero $Q$-probability | Two city maps agree on every street people actually drive on. |
| **Metric ($d$)** | True Distance Metric | Satisfies 4 axioms: non-negativity, identity, symmetry, triangle | An official road distance table between physical cities. |
