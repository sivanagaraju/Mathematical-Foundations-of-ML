...
| :--- | :--- | :--- | :--- |
| **$p(x)$** | Probability Density Function (PDF) | Continuous height function whose area over a region gives probability | The thickness of butter spread across a slice of toast. |
| **$\mathcal{D} \sim_{\text{iid}} p_x$** | Empirical training sample dataset | A finite collection of $n$ raw data points sampled from unknown law | A jar of 1,000 caught raindrops, not the cloud itself. |
| **$G_\theta: \mathcal{Z} \to \mathcal{X}$** | Implicit neural generator mapping | Deep network converting standard noise $Z$ into synthetic samples $\hat{X}$ | A pasta extruder turning plain dough into shaped noodles. |
| **$u = \frac{p(x)}{q(x)}$** | Likelihood / Density Ratio | Relative likelihood ratio comparing distribution $P$ against $Q$ at $x$ | A chemical litmus test comparing acid vs base balance. |
| **$D_f(P \parallel Q)$** | $f$-Divergence Discrepancy | Expected discrepancy $\mathbb{E}_{x \sim Q}[f(p(x)/q(x))]$ between $P$ and $Q$ | A custom spring stretched between two probability hills. |
| **$f''(u) \ge 0$** | Convex Generator Function | Upward-curving bowl function satisfying $f(1)=0$ | A parabolic skate park ramp whose base touches zero. |
| **$D_{\text{KL}}(P \parallel Q)$** | Forward Kullback-Leibler Divergence | $\int p(x) \ln \frac{p(x)}{q(x)} dx$ (Penalizes missing data modes) | An emergency parachute that must stretch over all mountains. |
| **$D_{\text{RKL}}(P \parallel Q)$** | Reverse KL Divergence | $\int q(x) \ln \frac{q(x)}{p(x)} dx$ (Penalizes generating fake junk) | A laser-focused flashlight that only shines on safe ground. |
| **$\text{JSD}(P \parallel Q)$** | Jensen-Shannon Divergence | Symmetrical, smoothed mixture divergence bounded in $[0, \ln 2]$ | A fair treaty compromising equally between two opposing nations. |
