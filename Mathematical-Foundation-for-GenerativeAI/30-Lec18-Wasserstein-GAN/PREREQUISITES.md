...
| Word | Picture | This hour’s job |
|------|---------|-----------------|
| $p_x$ | real pile (training images) | the law you wish to match |
| $p_\theta$ / $p_{\hat x}$ | fake pile from $G_\theta(z)$ | the law you can sample |
| support | grass that actually has sand | not the whole football field $\mathbb{R}^d$ |
| manifold | a thread / paper in a room | thin sheet, **not** a linear subspace |
| $D_f$ | $f$-divergence (KL, JSD, …) | last hour’s yardstick; can **max out** |
| $W$ | Wasserstein / earth-mover | least shovel-work between piles |
| $\pi$ | transport **plan** = joint table | one way to shovel $p_x$ into $p_{\hat x}$ |
| $T_w$ | critic net | scores a point; this hour must be **1-Lipschitz** |
| saddle | $\min_\theta\max_w$ of one $J$ | still the training problem (hence **WGAN**) |
