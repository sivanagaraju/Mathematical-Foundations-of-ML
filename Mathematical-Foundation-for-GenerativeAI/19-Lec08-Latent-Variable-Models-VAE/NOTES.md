...
| :--- | :--- | :--- | :--- | :--- |
| **$x \in \mathbb{R}^D$** | Observable Data Vector | `x = batch_images` | Observed high-dimensional pixel input | [Tensors & Shapes](../../MathsTerms/02-Linear-Algebra-Geometry-and-Tensors/04-Tensors_and_Shapes.md) |
...
| **$p_\theta(x) = \int p(x,z) dz$** | Marginal Likelihood (Evidence) | Incomplete log-likelihood target | Objective to maximize via lower bound | [Likelihood & Log-Likelihood](../../MathsTerms/04-Probability-and-Statistical-Estimation/04-Likelihood_and_Log_Likelihood.md) |
| **$p_\theta(z \mid x)$** | True Posterior Distribution | $\frac{p_\theta(x \mid z) p(z)}{p_\theta(x)}$ | Intractable Bayesian posterior over latent variables | [Joint, Marginal & Conditional Dist](../../MathsTerms/04-Probability-and-Statistical-Estimation/03-Joint_Marginal_Conditional_Dist.md) |
