...
| :--- | :--- | :--- | :--- | :--- |
| $\mathbf{X} \in \mathbb{R}^{N \times T \times D}$ | 3D Mini-Batch Sequence Tensor | `x = torch.randn(N, T, D)` | 3D sequence layout (`batch_first=True`). | [Tensors & Shapes](../../MathsTerms/02-Linear-Algebra-Geometry-and-Tensors/04-Tensors_and_Shapes.md) |
| $\mathbf{x}_t \in \mathbb{R}^D$ | Current Time-Step Observation | `x[:, t, :]` | Input vector at time step $t$. | [Tensors & Shapes](../../MathsTerms/02-Linear-Algebra-Geometry-and-Tensors/04-Tensors_and_Shapes.md) |
...
| $\mathbf{f}_t, \mathbf{i}_t, \mathbf{o}_t$ | LSTM Gate Activations ($\in [0, 1]$) | Computed internally in `nn.LSTM` | Sigmoid valves controlling erase, write, and read. | [Activation Functions](../../MathsTerms/03-Multivariate-Calculus-and-Optimization/05-Activation_Functions.md) |
| $\mathbf{r}_t, \mathbf{z}_t$ | GRU Gate Activations ($\in [0, 1]$) | Computed internally in `nn.GRU` | Reset and update gates in GRU. | [Activation Functions](../../MathsTerms/03-Multivariate-Calculus-and-Optimization/05-Activation_Functions.md) |
...
| $\mathbf{z} \in \mathbb{R}^{N \times C}$ | Unnormalized Output Logits | `logits = model(x)` | Class score matrix fed into `nn.CrossEntropyLoss`. | [Softmax](../../MathsTerms/03-Multivariate-Calculus-and-Optimization/06-Softmax.md) |
