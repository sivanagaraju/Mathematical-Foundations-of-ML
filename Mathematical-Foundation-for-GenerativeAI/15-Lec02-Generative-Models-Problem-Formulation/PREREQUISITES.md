...
| Symbol / Term | Formal Concept | Plain-English Software Meaning | Everyday Physical Metaphor |
| :--- | :--- | :--- | :--- |
| **$\text{RE}$** | Random Experiment | An empirical process with non-deterministic outcomes. | Shuffling and dealing a deck of cards or snapping a photo. |
| **$\Omega$** | Sample Space | Set of all possible abstract physical outcomes. | The backstage universe of infinite possible theatre plays. |
| **$\mathcal{F}$** | Event Space ($\sigma$-algebra) | Collection of valid subsets/questions we can assign probability to. | The official rulebook of allowed bets in a casino. |
| **$P$** | Probability Measure | Function $P: \mathcal{F} \to [0, 1]$ assigning likelihoods to events. | A master scale measuring the total mass of gold in a vault. |
| **$X: \Omega \to \mathbb{R}^d$**| Random Variable | A deterministic function mapping outcomes to numeric vectors. | A digital barcode scanner converting products into numeric strings. |
| **$\mathbb{R}^d$** | Euclidean Vector Space | A continuous space with $d$ orthogonal numerical axes. | A spreadsheet with $d$ distinct numeric columns. |
| **$\operatorname{Range}(X)$** | Image / Range of $X$ | The subset of $\mathbb{R}^d$ that the measurement function can hit. | The collection of all possible printable passport photos. |
| **$p_X(x)$ or $p_x$** | Probability Distribution | The probability density/mass function of data vectors $x \in \mathbb{R}^d$. | The official historical climate file of a city. |
| **$D = \{x_1, \dots, x_n\}$** | Dataset | A collection of $n$ observed vector realizations in $\mathbb{R}^d$. | A physical barrel of crude oil waiting to be refined. |
| **$\sim p_x$** | Sampled According To | Shorthand asserting vectors were generated by the unknown law $p_x$. | Drawing balls from a specific lottery urn. |
| **$\text{IID}$** | Independent & Identically Distributed | Each sample draw is independent and governed by the exact same law. | Flipping the exact same fair coin repeatedly without bias. |
| **$p_\theta$** | Parametric Model Family | An assumed mathematical/neural network family parameterized by $\theta$. | A programmable music synthesizer with tunable knobs $\theta$. |
| **$d(p_x, p_\theta)$** | Divergence / Distance Metric | A mathematical score quantifying the discrepancy between two distributions. | A ruler measuring the geometric gap between two clouds of smoke. |
| **$\theta^* = \arg\min_\theta d$**| Parameter Training | Finding the optimal parameters that minimize the divergence gap. | Tuning a radio dial until static noise drops to zero. |
