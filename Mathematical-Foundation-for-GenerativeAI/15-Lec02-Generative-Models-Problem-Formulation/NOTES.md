...
| :--- | :--- | :--- | :--- | :--- |
| **$(\Omega, \mathcal{F}, P)$** | Kolmogorov Probability Triplet | The abstract model of nature generating uncertain physical events. | The hidden physical world outside the computer. | [Probability Basics & Axioms](../../MathsTerms/01-Primal-Analysis-and-Foundations/Probability_Basics_and_Axioms.md) |
| **$\omega \in \Omega$** | Elementary Outcome | A single physical realization of the universe. | A real-world human being standing before a camera. | [Probability Basics & Axioms](../../MathsTerms/01-Primal-Analysis-and-Foundations/Probability_Basics_and_Axioms.md) |
| **$X: \Omega \to \mathbb{R}^d$** | Random Variable (Random Vector) | Deterministic mapping from physical outcome to digital vector. | `sensor.capture(real_world_scene)` | [Random Variables & Distributions](../../MathsTerms/04-Probability-and-Statistical-Estimation/Random_Variables_and_Distributions.md) |
| **$\mathbb{R}^d$** | $d$-Dimensional Euclidean Space | Continuous vector space where data coordinates live. | `torch.Tensor(size=(d,))` | [Tensors & Shapes](../../MathsTerms/02-Linear-Algebra-Geometry-and-Tensors/Tensors_and_Shapes.md) |
| **$m \times n \to \mathbb{R}^{mn}$** | Pixel Grid Flattening | Stacking image rows into a single continuous vector. | `image_tensor.view(-1)` or `img.flatten()` | [Tensors & Shapes](../../MathsTerms/02-Linear-Algebra-Geometry-and-Tensors/Tensors_and_Shapes.md) |
| **$e_k \in \mathbb{R}^v$** | One-Hot Token Vector | Binary vector of length $v$ with a 1 at index $k$. | `torch.nn.functional.one_hot(token_id, v)` | [One-Hot Encoding](../../MathsTerms/02-Linear-Algebra-Geometry-and-Tensors/One_Hot_Encoding.md) |
| **$\operatorname{Range}(X)$** | Range Space / Image of $X$ | Subset of $\mathbb{R}^d$ containing all valid outputs of $X$. | Manifold of valid natural data in $\mathbb{R}^d$. | [Random Variables & Distributions](../../MathsTerms/04-Probability-and-Statistical-Estimation/Random_Variables_and_Distributions.md) |
| **$p_X(x)$ or $p_x$** | True Probability Density Function | The unknown true probability distribution of data in $\mathbb{R}^d$. | The ground-truth data distribution. | [Common Probability Distributions](../../MathsTerms/04-Probability-and-Statistical-Estimation/Common_Probability_Distributions.md) |
| **$D = \{x_1, \dots, x_n\}$** | Dataset of Realizations | Finite collection of $n$ observed vector realizations. | `train_loader` / NumPy dataset array. | [Common Probability Distributions](../../MathsTerms/04-Probability-and-Statistical-Estimation/Common_Probability_Distributions.md) |
| **$x_i \sim p_x$** | Sampled According To | Shorthand asserting $x_i$ was drawn from unknown law $p_x$. | `x_i = sample_from_nature()` | [Common Probability Distributions](../../MathsTerms/04-Probability-and-Statistical-Estimation/Common_Probability_Distributions.md) |
| **$p_\theta(x)$** | Parametric Model Family | Assumed mathematical distribution family with parameters $\theta$. | `model = NeuralDensityEstimator(params)` | [Maximum Likelihood Estimation](../../MathsTerms/04-Probability-and-Statistical-Estimation/MLE.md) |
| **$\theta \in \Theta$** | Model Parameters | Trainable weights and biases of the neural network/model. | `model.parameters()` | [Gradient Descent](../../MathsTerms/03-Multivariate-Calculus-and-Optimization/Gradient_Descent.md) |
| **$d(p_x, p_\theta)$** | Statistical Divergence | Discrepancy metric scoring distance between true and model laws. | `criterion(p_data, p_model)` / Loss function. | [KL Divergence](../../MathsTerms/05-Information-Theory-and-Divergences/KL_Divergence.md) |
| **$\theta^* = \arg\min_\theta d$** | Optimal Parameter Vector | Parameters that minimize divergence between model and truth. | Output of `optimizer.step()` after convergence. | [Gradient Descent](../../MathsTerms/03-Multivariate-Calculus-and-Optimization/Gradient_Descent.md) |
| **$\hat{x} \sim p_{\theta^*}$** | Generative Sampling | Simulating the random experiment to mint new synthetic data. | `synthesized_img = generator.sample(z)` | [Random Variables & Distributions](../../MathsTerms/04-Probability-and-Statistical-Estimation/Random_Variables_and_Distributions.md) |
